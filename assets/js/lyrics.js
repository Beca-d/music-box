...
const songID = async (event) => {
	...
	let isFeatTrack =
		track.indexOf("(") !== -1 ? track.split("(")[0].trim() : track;
...
var isResponseOK = (response) => {
	if (!response.ok) throw Error("No Lyrics Found !");
	else return response.json();
};

// Data Validation
var isEmptyOrNull = (data) => {
	if (data === null || data === "") throw Error("No Records Found !!");
	else return data;
};
...
document.addEventListener("DOMContentLoaded", () => {
	...
	(
		document.querySelectorAll(
			".modal-background, .close-lyrics, .close-lyrics-not-found"
		) || []
	).forEach(($close) => {
...
document.querySelector(".copier").onclick = function () {
	const _this = this;
	navigator.clipboard
		.writeText(document.getElementById("song-lyrics-modal").innerText)
		.then(function () {
